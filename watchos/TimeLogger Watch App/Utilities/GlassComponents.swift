...
struct AnimatedMesh: View {
        ...
        if #available(iOS 18.0, macOS 15.0, watchOS 11.0, *) {
            meshView
                ...
                .onChange(of: scenePhase) { _, phase in
                    if phase == .active { startIfNeeded() } else { t = 0 }
                }
        } else {
            RadialGradient(
    ...
    private func startIfNeeded() {
        guard animated, !ProcessInfo.processInfo.isLowPowerModeEnabled else { return }
    ...
    private var meshView: some View {
        ...
        return MeshGradient(
            width: 3, height: 3,
            points: [
                [0, 0], [0.5, 0 + 0.04 * t], [1, 0],
                [0, 0.5 + 0.08 * t], [0.5, 0.5], [1, 0.5 - 0.08 * t],
                [0, 1], [0.5, 1 - 0.05 * t], [1, 1]
            ],
            colors: [
                base.opacity(0.70), base.opacity(0.45), base.opacity(0.15),
                base.opacity(0.35), base.opacity(0.65), base.opacity(0.25),
                base.opacity(0.10), base.opacity(0.35), base.opacity(0.55)
            ]
        )
    }
}
...
struct TransportBadge: View {
    ...
    /// Convenience init from a SyncTransport value.
